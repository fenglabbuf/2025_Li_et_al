...
def compute_grn_centrality(DG, modules, random_s=10):
    ...
    d_btw = nx.betweenness_centrality(DG, seed=random_s)
    ...
    d_btw = nx.betweenness_centrality(DG)
    d_odeg = nx.out_degree_centrality(DG)
    ...
    return pd.concat([df], keys=[len(df)], names=['nodes', 'geneid'])